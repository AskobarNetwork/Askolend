...
export async function createToken(
  moneyMarket: MoneyMarketInstanceService
): Promise<Token> {
  const provider = moneyMarket.provider;
  const highRisk = new AskoRiskTokenService(
    provider,
    await moneyMarket.getHighRisk()
  );
  const lowRisk = new AskoRiskTokenService(
    provider,
    await moneyMarket.getLowRisk()
  );
  ...
  let lowRiskBalance = fromWei(
    await lowRisk.viewConvertFromART(await lowRisk.getBalance(userAddress))
  );
  let highRiskBalance = fromWei(
    await highRisk.viewConvertFromART(await highRisk.getBalance(userAddress))
  );
  ...
  try {
    ...
    let borrowedAHR = fromWei(await highRisk.borrowBalancePrior(userAddress));
    console.log("BORROWEDALR ", Number(borrowedALR).toString());
    console.log("BORROWEDAHR ", Number(borrowedAHR).toString());
  ...
  try {
    borrowLimit = fromWei(
      await lowRisk.viewUSDCWorthOfART(
        await lowRisk.viewConvertFromART(await lowRisk.getBalance(userAddress))
      )
    );
  } catch (ex) {
    console.log("view usdc worth of art limit error", ex);
  }

  const canSupply = await asset.hasEnoughAllowance(
    userAddress,
    moneyMarket.address,
...
export function getTokenLogoPngSrc(address: string): string {
  return new URL(
    (process.env.REACT_APP_ASSETS_ENDPOINT || "") + address + "/logo.png"
  ).toString();
}
