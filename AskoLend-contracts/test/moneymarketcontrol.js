...
contract("MoneyMarketControl", (accounts) => {
  ...
  it("should return the right amount of ART for each lend", async () => {
    await link.transfer(account_two, "1000000000000000000000000")
    await link.approve(linkMMI.address, "1000000000000000000000000", {
      from: account_one,
    });
    await link.approve(linkMMI.address, "1000000000000000000000000", {
    ...
    await linkMMI.lendToAHRpool(web3.utils.toWei("1000"), {from: account_one});
    console.log("account one lends 1000 the first time ")
    let linkAHRbal = await linkAHR.balanceOf(account_one);
    let linkBalafter = await link.balanceOf(account_one);
    console.log(
      "account one receives " + web3.utils.fromWei(linkAHRbal) + "  for the first 1000 lent"
    );
    linkAHRbalValAsset = await linkAHR.viewConvertFromART(linkAHRbal);
   console.log("the value of account one's AHR is: " + web3.utils.fromWei(linkAHRbalValAsset, "ether"));
///////////////////////////////////////////////////////////////////////////////////
console.log("account one lends another 1000 link")
    await linkMMI.lendToAHRpool(web3.utils.toWei("1000"), {from: account_one});
    linkAHRbal = await linkAHR.balanceOf(account_one);
    console.log(
      "account one Link AHR bal after 2000 lent is: " + web3.utils.fromWei(linkAHRbal)
    );
    linkAHRbalValAsset = await linkAHR.viewConvertFromART(linkAHRbal);
   console.log("the value of the account one's AHR after 2000 lend is: " + web3.utils.fromWei(linkAHRbalValAsset, "ether"));
///////////////////////////////////////////////////////////////////////////////////
console.log("so far so good. Now account two is gonna lend 1000")
await linkMMI.lendToAHRpool(web3.utils.toWei("1000"), {from: account_two});
linkAHRbal = await linkAHR.balanceOf(account_two);
console.log(
  "account two Link AHR bal after first lend: " + web3.utils.fromWei(linkAHRbal)
);
linkAHRbalValAsset = await linkAHR.viewConvertFromART(linkAHRbal);
console.log("the value of the account two AHR is: " + web3.utils.fromWei(linkAHRbalValAsset, "ether"));
///////////////////////////////////////////////////////////////////////////////////
console.log("somehow account two's AHR balance looses value while account ones gains the missing value")
linkAHRbal = await linkAHR.balanceOf(account_one);
console.log(
  "Link account one AHR bal after second lend: " + web3.utils.fromWei(linkAHRbal)
);
linkAHRbalValAsset = await linkAHR.viewConvertFromART(linkAHRbal);
console.log("the value of the AHR account one is: " + web3.utils.fromWei(linkAHRbalValAsset, "ether"));
  });
  /////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////
  ...
  // });
/////////////////////////////////////////////////////////////////////////////////
  //  it("should return the right amount of Link for ART after several borrows again because obsessive", async () => {
  //
  ...
  //   let availibleCollat = await MMC.viewAvailibleCollateralValue(account_one, augurALR.address, {from: account_one})
  //   console.log("availible Collateral Value for augur after lending: " + availibleCollat)
  ...
  //     }
  //   );
  //    availibleCollat = await MMC.viewAvailibleCollateralValue(account_one, augurALR.address, {from: account_one})
  //   console.log("availible Collateral Value for augur after borrowing: " + availibleCollat)
  //   console.log("time to time travel  into the world of tommorow!");
  //   let numOfBlock = 50;
  //
  //   for (let block = 0; block < numOfBlock; ++block) {
  ...
  //
  // console.log("the link balance of account four Before lend/borrow/redeem: " + web3.utils.fromWei(linkb4bal))
  // console.log("the link balance after lend: " + web3.utils.fromWei(linkAfterLend))
  // console.log("the link balance of account four After lend/borrow/redeem:  " + web3.utils.fromWei(linkAfterRedeem))
  //
  // });
});
